...
\begin{enumerate}
\item \label{orgcb5235f} | main address for the iteration process.  SC received the data values (OSCdef).  Py → SC
\item \label{org8569b95} | controls the synths, acts like an ON/OFF switch.  Triggered by the 6 synth ON/OFF buttons. Py → SC
\item \label{orgd968779} | This acts like main ON/OFF switch for the all synths, it is triggered by the START and Killall buttons. Py → SC
\item \label{org4f723a1} | Initialisation address, activates the interaction elements on the interface when (or if) the SC responds.  This configuration uses the 1234 port instead. SC → Py
\end{enumerate}
...
\label{sec:orge43afeb}
\subsubsection{Map to Lydian scale}
\label{sec:org7266e50}
